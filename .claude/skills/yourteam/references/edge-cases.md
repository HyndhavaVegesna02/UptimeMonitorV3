...
- **No subagent capability** (environment without a Task/agent tool): run the same pipeline inline — same prompts read as checklists, same DoD gate, same evidence. The roles survive even when the isolation doesn't; note the degradation to the PO once.
