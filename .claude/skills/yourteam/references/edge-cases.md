...
## 13. Trivial interrupted tail — the orchestrator may finish directly

When an implementer subagent is interrupted (crash, connection drop, session limit) leaving only a trivial remainder AND a committed failing test already pins the contract for that remainder, the orchestrator may complete it directly — after the usual verify-the-tree step (preserve coherent work, discard scraps) — rather than burn a fresh dispatch. Record the completion in the story's plan/board note; the full DoD gate (plus reviewers, if the story's size requires them) applies unchanged. Anything beyond a trivial tail → the fresh-agent rule stands. (Migrated 2026-07-12 from the 2026-06-25 working agreement; motivated by Sprint 5, STORY-020: a ~4-line completion after a session-limit death.)
