...
Implementers follow TDD with a commit after every green step — that commit cadence *is* the crash-recovery mechanism, and you police it: at every board touch, check `git log` recency on the sprint branch; a silent gap over ~30 minutes means a stalled agent — kill it, verify the tree (preserve coherent work, discard scraps), dispatch fresh. Tick `plan.md` checkboxes and update the board at every transition. You are the sole writer of `.scrum/` state.
