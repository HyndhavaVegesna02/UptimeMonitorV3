 ...
 *
 * Re-evaluating `matches` when `query` itself changes uses the React-
 ...
 * `useEffect`) — the same pattern `SampleModeBanner.tsx` already uses, and
 * required here since `eslint-plugin-react-hooks`'s `set-state-in-effect`
 * rule (DoD gate) rejects a synchronous `setState` in an effect body.
 */
