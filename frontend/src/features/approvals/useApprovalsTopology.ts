...
/**
 * Fetch hook for `GET /api/v1/topology` (STORY-100 AC1) — feeds the
 * Approvals tab's evidence-first card: the friendly component name and the
 * component's primary signal (`useProposalEvidence`'s input), resolved by
 * joining a proposal's `component_id` against this topology. A thin
 * `useFetch` wrapper, mirroring `features/dashboard/useTopology.ts`'s
