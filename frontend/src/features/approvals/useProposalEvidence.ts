...
/** One "latest result at this location" row for a proposal card (STORY-100
 * AC1): status + latency + the raw observed-at instant (rendered via the
 * shared `RelativeTime`). */
...
/**
 * Per-proposal evidence hook (STORY-100 AC1, AC4): "latest result per
 * location" for the proposal's PRIMARY topology signal, on the EXISTING
 * `GET /api/v1/history`. `ProposalDTO`/`StatusProposal` carry no
