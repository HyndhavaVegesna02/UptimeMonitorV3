...
8. **Fixture-cardinality gap ("not realistic on the wire")** — the fixture omits a shape the REAL wire produces (duplicate keys, repeated timestamps, colliding tuples, empty/'' values), so a rendering/identity bug that only fires on real cardinality passes every green test AND both reviewers. A reviewer's claim that a collision or edge is "not realistic on the wire" is a HYPOTHESIS, not evidence — it must be settled by the live reality gate against real data (real duplicates, real row counts), never waved off. For any list/table/keyed rendering, verify identity/uniqueness against real wire cardinality (sprint-60 STORY-130: live `/history` had duplicate `(signal_key, observed_at, location)` triples → React key collision → visibly corrupted filtered rows; spec PASS + quality APPROVE both passed it, quality having dismissed the collision as "not realistic on the wire"; only the reality gate caught it).
