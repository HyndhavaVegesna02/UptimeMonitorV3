# Sprint 50 Plan â€” Deploy for real: live console deployment + sprint-49 hardening

- **Sprint goal:** Harden the container image and stack (STORY-093), then execute the
  live console deployment and verify the deployed system end-to-end (STORY-089) â€”
  completing the AWS migration epic's deployment leg.
- **Mode:** `in-process` (STORY-093 via yt-implementer; STORY-089 is PO-driven â€” console
...
- **Stories & order:**
  1. **STORY-093** (2 pts) â€” sprint-49 review minors: container hardening, ECS health
  ...
  2. **STORY-089** (3 pts) â€” live console deployment + e2e verification (PO-driven).
- **Plan-verifier:** DISPATCHED â€” the sprint is contract-sensitive (STORY-089 exercises
...
## STORY-093 â€” Chore: sprint-49 review minors (2 pts)
...
- `Dockerfile:14` today: `RUN pip install --no-cache-dir . uvicorn[standard]` runs AFTER
  `COPY backend /app/backend` (line 10) â€” any source edit invalidates the pip layer.
  ...
  `pyproject.toml` alone â€” e.g. a stdlib `tomllib` one-liner emitting the
  `[project] dependencies` list to pip â€” BEFORE `backend/` is copied; the final
...
- `infra/stack.yaml:381` `APIService` has no `HealthCheckGracePeriodSeconds`;
  ...
  unhealthy after 3 fails (`stack.yaml:364-368`) â‰ˆ 90s budget; the boot lifespan seed
...
- Guard-test scope (plan step-4 invariant from sprint-49): no file under `backend/src`
  may contain `sqlalchemy`, `create_engine`, or `psycopg` â€” scan file contents
...
- [x] 1. Test hygiene first (pure-test commit): add the no-Postgres guard test
  ...
  `psycopg` appears); run it â€” green (invariant holds today). Commit.
- [x] 2. Give `test_main_resource_lifecycle_success` real assertions: after
  ...
  `(config, db_resource, "uptime-control")` â€” the real call shape is
  `seed_topology_dynamo(config, db_resource, settings.dynamo_control_table)`
  (`run.py:202`; default table `"uptime-control"`, `settings.py:22`) â€” and
...
- [x] 5. Reality check the image (adapter-type story â‡’ live probe): `docker build`
  twice with a whitespace-only `backend/` source touch between â€” assert the dependency
  ...
  `whoami` inside the container â†’ the non-root user. Record output. Commit any fix.
- [x] 6. `infra/stack.yaml`: add `HealthCheckGracePeriodSeconds: 120` to
  `APIService.Properties`; run `cfn-lint infra/stack.yaml` â†’ exit 0. Commit.
- [x] 7. Runbook branch reconcile: update `docs/deploy-runbook.md` Prerequisite 4
  ...
  branch HEAD â€” a PO executing the runbook verbatim must deploy the template and
...
- [x] 8. Story gate: `yt_gate.py --only` scoped to pytest + ruff + cfn-lint (diff
  touches backend tests, Dockerfile, stack.yaml â€” no frontend). Wiki blast radius:
  `deployment-and-infra.md` / `deployment` articles citing the Dockerfile or
  `stack.yaml` get updated or re-verified. Board â†’ done pending review.
  (Ran the full 8-command DoD gate, incl. import-linter + frontend, per AC4's
  "full amended DoD gate ... + frontend" â€” all 8 green at a8700f5.)
...
## STORY-089 â€” Live console deployment + e2e verification, PO-driven (3 pts)

The PO executes `docs/deploy-runbook.md` (Steps 1â€“5) in the AWS console; the
...
- Runbook: `docs/deploy-runbook.md` â€” Step 1 stack creation, Step 2 Secrets Manager
...
- Secrets: `DYNATRACE_ENV_URL`/`DYNATRACE_API_TOKEN`/`STATUSPAGE_PAGE_ID`/
  ...
  are injected by the task definitions (runbook Â§"Environment variables injected by
  the stack").

### Steps (each â–¶ is a sanctioned PO console action; each âœ” is orchestrator verification)

- [x] 1. Preflight: STORY-093 done on the sprint branch; the PO's checkout for all
  runbook steps is the `sprint-50` HEAD that includes STORY-093 (runbook
  Prerequisite 4 updated by STORY-093 step 7 â€” never sprint-49); `cfn-lint` green;
  `docker build` green locally; frontend `npm run build` green.
- [x] 2. â–¶ PO runs runbook Step 1 (stack create) + Step 2 (secret values).
  âœ” AC1a: stack `CREATE_COMPLETE`; secrets exist; no secret value pasted anywhere
  in the repo/chat transcript that would be committed (AC5).
- [x] 3. â–¶ PO runs runbook Step 3 (ECR push â€” image built from the sprint-branch
  HEAD including STORY-093) and forces new deployments if services pre-started.
  âœ” AC1b: both ECS services steady (api task healthy in the target group; loop
  DesiredCount 1 with exactly one RUNNING task).
- [x] 4. â–¶ PO runs runbook Step 4 (SPA build + S3 upload + CloudFront invalidation,
  unconditional per runbook Step 4.4). âœ” AC2: CloudFront URL over HTTPS renders all six tabs; browser
  network panel shows `/api/*` served same-origin (no CORS errors).
- [x] 5. âœ” AC3 (live loop): watermark item in the control table advances across â‰¥2
...
- [x] 6. â–¶+âœ” AC4 (mutation round-trip): from the deployed UI, toggle sample-mode ON
  â†’ verify the control-table item flipped in DynamoDB (console or CLI) â†’ toggle OFF
  and re-verify (leaves prod clean). Fallback path: maintenance schedule + delete.
- [x] 7. âœ” AC5 sweep: `git log -p sprint-50 --` scan (and `git grep` at HEAD) for any
  secret value committed during the sprint â†’ none.
- [x] 8. AC6 docs: append the deployed-topology section to CLAUDE.md (stack name,
...
- [x] 9. Story gate: full amended 8-command gate on final HEAD (this is also the
  sprint-close full gate) + wiki sweep (`yt_wiki.py` exit 0).

### Reality gate (089)
The story IS a reality gate â€” every AC is a live probe of the deployed stack; evidence
...
- Stack create fails â†’ PO deletes the stack (DeletionPolicy Retain protects the two
