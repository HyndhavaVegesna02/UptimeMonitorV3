...
## Acceptance Criteria
- [x] `backend/src/api/v1/_shared/windowing.py` exposes the window-defaulting policy (the 24h
      ...
      moves.
- [x] `availability/service.py` and `history/service.py` both consume it; NO private copy of the
      defaulting logic remains in either (the duplicated constant + defaulting expressions are
      deleted, not shadowed).
- [x] `windowing.py` has direct unit tests covering: both defaults applied, one-sided defaults,
...
- [x] **Frozen behavior:** all existing endpoint tests pass UNMODIFIED (window semantics are
      observable through them).
- [x] Backend six-gate DoD green (lint-imports still 8 kept, 0 broken — `_shared` already fenced);
...
- 2026-07-10: Implemented in sprint-42. All tests pass, duplication of defaulting logic removed, and wiki updated. final commit: `3357bd2` (and dependencies).
