...
- [ ] AC1: Given a `native_id` containing a `"` (or other DQL-breaking character),
      ...
      query.
- [ ] AC2: A well-formed `native_id` still builds the same query as today (no regression);
      a test covers both the rejecting case and the unchanged happy path.
- [ ] AC3: `lint-imports` stays green; the existing STORY-008 query-builder tests still pass
      unchanged.
