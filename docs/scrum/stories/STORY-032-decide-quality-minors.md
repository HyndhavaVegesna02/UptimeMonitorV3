...
- [x] AC1: The two degradation branches in `decide` (no-open-proposal vs supersede) no longer
...
- [x] AC2: `opened.id` (typed `int | None`) is guarded before being passed to `resolve(proposal_id:
...
- [x] AC3: Import style matches the peer core services (import the severity helpers from the same
...
- [x] AC4: Behaviour unchanged — all existing `test_decide.py` tests pass unchanged; `pytest` +
