...
- [ ] AC1: Remove the leftover import-smoke test `test_publisher_can_be_imported` in
      `backend/tests/test_statuspage_adapter.py` (it only asserts `is not None`; real tests now cover
      the adapter).
- [ ] AC2: Resolve the unused `backend/tests/fixtures/statuspage/component_degraded.json` — EITHER
...
- [ ] AC3: Tidy the style nits flagged in review: add the missing blank line between
...
- [ ] AC4: `pytest`, `lint-imports` green; no behaviour change to production code.
