...
- [x] AC1: `ruff` is added to the dev extras in `pyproject.toml` and configured (a `[tool.ruff]`
...
- [x] AC2: The existing tree is formatted/import-sorted in ONE mechanical pass (`ruff format` +
...
- [x] AC3: `ruff check` and `ruff format --check` are wired into `.scrum/definition-of-done.md` as
...
- [x] AC4: All DoD gates green after the change: the four existing (`pytest`, `lint-imports`,
