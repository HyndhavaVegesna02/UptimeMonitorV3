...
- [ ] AC1: `dev_db.py up` removes any pre-existing container of its target name (`docker rm -f
...
- [ ] AC2: `up` is idempotent: running `up` when a healthy container of that name is already running
...
- [ ] AC3: `down` still removes the container cleanly. A real port conflict from a DIFFERENT process
...
- [ ] AC4: `lint-imports` + `pytest` stay green; if `dev_db.py` has unit-testable helpers, the new
