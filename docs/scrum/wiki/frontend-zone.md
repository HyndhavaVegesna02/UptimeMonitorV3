...
- **`src/` layout:**
  ...
  - `pages/` — `DashboardPage` is the only REAL page (STORY-122); `Availability/History/Approvals/
    Maintenance/Publications` are `PlaceholderPage` stubs awaiting sprints 60–61; `StyleguidePage`
    renders every primitive × state (STORY-120).
  ...
  - `mocks/` — MSW is the ONLY mocked I/O edge; per-endpoint handlers
    (`handlers/{components,approvals,history,availability,maintenance}.ts`) composed in
...
- **API client (`api/client.ts`):** `getComponents()`, `getApprovals()`, `getHistory(signalKey,
  limit?)`, `getAvailability(signalKey)`, `getMaintenance()`. DTOs in `api/types.ts`:
  `ComponentDTO {id,name,status}`, `ProposalDTO`, `ObservationDTO {signal_key, observed_at,
  health, location, latency_ms, response_status_code, check_type}`, `AvailabilityDTO`
  (`availability_pct` is a 0–1 fraction, nullable), `MaintenanceWindowDTO`. Re-derived fresh from
  the LIVE contracts (verified at the STORY-121/122 reality gates); extends cleanly as later tabs
...
- The five non-Dashboard tabs are `PlaceholderPage` stubs — real content is sprints 60–61.
...
- STORY-125 (mobile-sheet focus-trap + `role="dialog"`), STORY-126 (skip-to-content link),
  ...
  STORY-128 (lazy-load the availability-dependent KPI + dedupe the components/approvals fetch).
