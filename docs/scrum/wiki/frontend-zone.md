...
status: stale
# DEMOTED 2026-08-12, not flagged by the sweep. Last read against code at sprint-63, six
# sprints ago, and it is the widest article in the wiki (67 code_refs over the whole
# frontend). The tier pass had to touch every article's frontmatter, and under the
# derived-baseline rule touching a `verified` article claims its Facts were re-read — so
# an article nobody re-read is marked stale instead of laundered. Readable and
# quarantined from briefs, which is the protocol's designed safe state, not a defect.
# The frontend rebuild programme (backlog, sprint-62 planning) should rehabilitate this
# article rather than the other way round: re-verify it when that work lands.
---
