...
- sprint-73 (STORY-147, this story's own AC4 evidence — NOT an unrelated-story sweep flag, unlike
  ...
  STORY-147 added two optional operator-cockpit display fields (`group`, `description`) to the
  `Component`/`ComponentConfig` types, and the new test is that same story's AC4 proof that
  neither field reaches Statuspage: `StatusChange` (the only type `publish()` reads) has exactly
  `{component_id, status}`, and the publish payload stays byte-identical
  (`{"component": {"status": ...}}`). `StatuspagePublisher.publish` itself, `build_publisher`, and
  every existing test in this file are unmodified. No Fact in this article changed.
